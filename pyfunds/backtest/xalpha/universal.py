...
# TODO: A suitable timescale for caching tokens
@lru_cache_time(ttl=300)
def get_token():
    """
Obtain the verification token of Xueqiu, which can also be obtained anonymously, and seems to be constant forever (it will change over a large time frame)
...
def get_portfolio_fromttjj(code, start=None, end=None):
    ...
    df["stock_ratio"] = df["stock_ratio"].replace("---", "0%").apply(lambda s: _float(s[:-1]))
    df["bond_ratio"] = df["bond_ratio"].replace("---", "0%").apply(lambda s: _float(s[:-1]))
    df["cash_ratio"] = df["cash_ratio"].replace("---", "0%").apply(lambda s: _float(s[:-1]))
...
def get_historical_fromsp(code, start=None, end=None, region="us", **kws):
    """
    S&P official website data source
...
def _get_daily(
    ...
    if not _from:
        if (code.startswith("SH") or code.startswith("SZ")) and code[2:8].isdigit():
            ...
            _from = "SPC"
        elif code.startswith("ZZ") and code[4:].isdigit():
            _from = "ZZ"
        elif code.startswith("GZ") and code[-3:].isdigit():
            _from = "GZ"
...
def get_cninvesting_rt(suburl, app=False):
    ...
    if suburl.startswith("commodities"):
        try:
...
def get_rt(
    ...
    :param double_check: Optional[bool], default False.

    :param handler: bool. Default True.
    :return: Dict[str, Any].
                ...
                if refresh:
                    ...
                    try:
                        ...
                        if df0.iloc[-1][date] < end_obj and not fetchonly:
                            ...
                            kws["end"] = end_str
                            if has_weekday(nextday_str, kws["end"]):
                                df2 = f(*args, **kws)
...
def get_fund_peb(code, date, threhold=0.3):
    ...
    :param code: str
    :param date:
    :param threhold: float, default 0.3.
    :return:
    """
    if code.startswith("F"):
        code = code[1:]
...
class vinfo(basicinfo, indicator):
    ...
    def __init__(
        ...
        **kws
        ...
        self.value_label = kws.get("value_label", 1)
        self.specialdate = []
        self.fenhongdate = []
        self.zhesuandate = []
        self.rate = rate
