...
class indicator:
    ...
    def bcmkset(self, infoobj, start=None, riskfree=0.0371724, name="COIN Portfolio"):
        """
    ...
    def correlation_coefficient(self, date=yesterdayobj()):
        ...
        correlation coefficient between aim and benchmark values,
          It is a good measure of how well the index is tracked
    ...
    def max_drawdown(self, date=yesterdayobj()):
        """
      The maximum drawdown for the backtesting period
    ...
    ## The above is basically the overall quantitative indicators provided by Jukuan, and the following are other short-term technical indicators
    ...
    def macd(self, fast_window=12, slow_window=26, signal_window=9, col="netvalue"):
        ...
        # The difference between short-term and long-term EMA
        MACDDiff = pd.Series(EMAfast - EMAslow)
        # The difference is again EMA averaged
        MACDDem = pd.Series(MACDDiff.ewm(span=signal_window).mean())
        # The difference between the average EMA and the original difference
    ...
    def mtm(self, window=10, col="netvalue"):
        """
     If you want to calculate the momentum average, use the ma or emca function, and select the MTM column for the col parameter
    ...
    def boll(self, window=10, deviation=2, col="netvalue"):
        """
        Bollinger Bands upper and lower band calculations
    ...
    def kdj(self, rsv_window=9, k_window=3, d_window=3, col="netvalue"):
        """
        KDJ StochasticSince this module does
        not involve information on intraday high and low prices, the highest and lowest prices of the range
        are replaced by extreme closing prices, so there may be discrepancies with the KDJ indicator calculated by other software.
    ...
    def wnr(self, window=14, col="netvalue"):
        """
       William's indicator, here the convention (wnr*-1) with an oversold result close to 0
       is in fact RSV, and the extreme price of the same range is replaced by the extreme closing price
    ...
    def dma(self, fast_window=10, slow_window=50, ama_window=10, col="netvalue"):
        """
        Parallel Line Difference Indicator
    ...
    def trix(self, window=10, ma_window=10, col="netvalue"):
        """
        Triple exponential smoothed average
    ...
    def psy(self, count_window=12, ma_window=6, col="netvalue"):
        """
       Psychometric (measures the number of days of gains over the past count_window days)
...
def plot_kline(
    ...
    """
    For the dataframe, the form of the upper and lower volume charts of the standard kanban software is directly drawn
