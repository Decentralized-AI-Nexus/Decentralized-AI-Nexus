...
def test_fundreport():
    ...
    assert r.get_report()[0][:2] == "guanf"
    assert r.analyse_report(1)["bank"][:2] == "ximgye"
    assert r.show_report_list(type_=0)[0]["FUNDCODE"] == "000827"
    assert r.get_report(id_="AN202003171376532533")[0][:2] == "guanf"
...
def test_fund():
    ...
    assert hs300.name == "jingsunchangc"
    assert hs300.fenhongdate[1] == pd.Timestamp("2017-08-15")
    assert hs300.get_holdings(2019, 4).iloc[0]["name"] == "peacec"
    assert (
    ...
    assert dax.feeinfo == ["Less than 7 days", "1.50%", "Greater than or equal to 7 days", "0.00%"]
...
def test_csvio():
    ...
    # there may be time lag for update of .js API, i.e. The API doesn't necessarily guarantee an update of yesterday's equity, even if it's not QDII
...
def test_fund_update():
    ...
    assert round(netvalue, 4) in netvaluel
