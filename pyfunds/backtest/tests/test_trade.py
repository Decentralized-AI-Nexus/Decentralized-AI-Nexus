...
def test_customize_fee():
    ...
    assert dqzf.dailyreport("2020-06-02")["Fund dividends and redemptions"].iloc[0] == 2.22
    assert dqzf.dailyreport("2020-05-31")["Hold shares"].iloc[0] == 116.5
    # issue 59
    df = pd.DataFrame(
    ...
    df["date"] = pd.to_datetime(df["date"])
    dqzf = xa.trade(xa.fundinfo("519732"), df)
    assert round(dqzf.dailyreport("2020-05-28").iloc[0]["The present value of the fund"], 0) == 10000.0
...
def test_ttjj_oversea_trade():
    ...
    assert df[df["基金代码"] == "968012"].iloc[0]["The total income of the fund"] == -20.01
...
def test_mulfix():
    ...
    assert round(tot.summary("2018-08-04").iloc[0]["Return on investment"], 1) == 1.0
...
def test_policy_scheduled():
    ...
    assert round(cm_t3.dailyreport("2018-08-03").iloc[0]["Return on investment"], 2) == -42.07
...
def test_policy_indicator_cross():
    ...
    assert round(cm_tt.dailyreport("2018-07-09").iloc[0].loc["Turnover rate"], 1) == 14.1
...
def test_policy_indicator_points():
    ...
    assert (
        zz500_t.dailyreport("2018-05-01").iloc[0].loc["The total income of the fund"] == -6302.32
    )  # -6302.26
...
def test_record_list():
    ...
    assert round(tot.combsummary("2019-05-04").iloc[0]["Return on investment"], 1) == 10.5
